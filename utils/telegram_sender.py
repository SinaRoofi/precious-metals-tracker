...
def calculate_bullion_rr(commodity, dfp):
    ...
    همیشه با GOLD_RR_YEAR_END_DAYS=365 (نه روز سپری‌شده‌ی سال جاری) —
    ...
    فرمول (مطابق ابزار Long Position در TradingView):
      ریسک (مشترک)     = قیمت فعلی − هدف(کف)    → فاصله‌ی رو به پایین تا کف (%)
      ریوارد (سقف)      = هدف(سقف) − قیمت فعلی   → فاصله‌ی رو به بالا تا سقف (%)
      ریوارد (ارزش)     = هدف(ارزش) − قیمت فعلی  → فاصله‌ی رو به بالا تا ارزش (%)
      R/R (سقف)         = ریوارد(سقف) / ریسک
      R/R (ارزش)        = ریوارد(ارزش) / ریسک

    توجه به جهت تفریق: ریوارد از «هدف منهای فعلی» و ریسک از «فعلی منهای
    هدف کف» حساب می‌شه — دقیقاً مثل TradingView، تا در ستاپ سالم (قیمت
    فعلی بالای کف) ریسک مثبت باشه. اگه قیمت زیر کف بیفته، ریسک منفی
    می‌شه (یعنی حتی بدترین سناریو هم هنوز بالادستی داره) — این حالت
    نامعمول ولی ریاضی‌اش درسته.

    ⚠️ عمداً بدون abs: نسبت‌ها می‌تونن منفی بشن و این عمدیه — علامت منفی
    ...
    risk = (current_price - value_low) / current_price * 100          # ریسک مشترک، رو به پایین تا کف
    reward_high = (value_high - current_price) / current_price * 100   # رو به بالا تا سقف
    reward_value = (value_value - current_price) / current_price * 100  # رو به بالا تا ارزش
    ...
    rr_high = safe_div(reward_high, risk)
...
def send_monthly_report(commodity, bot_token, chat_id):
    """
    ساخت و ارسال گزارش ماهانه‌ی یک کالا (دقیقاً مثل send_weekly_report، فقط برای ماه قبلی).

    Returns:
        True در صورت ارسال موفق، False در غیر این صورت (از جمله وقتی داده‌ی
        کافی برای ماه قبلی وجود نداشته باشد).
    """
    from utils.monthly_report import build_monthly_package  # جلوگیری از import چرخه‌ای

    if commodity not in CAPTION_ASSETS:
        raise ValueError(f"کالای نامعتبر: {commodity}")

    package = build_monthly_package(commodity)
    if package is None:
        logger.warning(f"⚠️ [{commodity}] داده‌ی کافی برای گزارش ماهانه وجود ندارد — ارسال لغو شد")
        return False

    try:
        url = f"https://api.telegram.org/bot{bot_token}/sendPhoto"
        files = {"photo": ("monthly_report.png", io.BytesIO(package["image_bytes"]), "image/png")}
        data = {"chat_id": chat_id, "caption": package["caption"], "parse_mode": "HTML"}
        response = requests.post(url, files=files, data=data, timeout=REQUEST_TIMEOUT)

        if response.status_code == 200:
            logger.info(f"✅ [{commodity}] گزارش ماهانه ارسال شد")
            return True

        logger.error(f"❌ [{commodity}] خطای ارسال گزارش ماهانه: {response.status_code} - {response.text}")
        return False

    except Exception as e:
        logger.error(f"❌ [{commodity}] خطا در ارسال گزارش ماهانه: {e}", exc_info=True)
        return False
...
def create_combined_image(commodity, Fund_df, last_trade, global_price, global_yesterday, dfp, yesterday_close):
    ...
    table_header = [
        ...
        "حباب ماه<br>(Avg)", "ورود پول", "ورود پول ماه", "سرانه خرید",
        "اختلاف سرانه", "ارزش معاملات", "بازده ماهانه", "بازده ماهانه<br>(قیمت-NAV)", "نماد",
    ]
    ...
    # 3: % آخرین | 4: % حباب | 5: حباب ماه(Avg) | 6: ورود پول | 7: ورود پول ماه
    ...
    cell_colors = [
        ...
        apply_gradient_colors(table_cells[5], vmin=vmin_5, vmax=vmax_5),   # حباب ماه(Avg)
    ...
    # عرض نسبی ستون‌ها: ستون‌هایی با هدر بلندتر (حباب ماه(Avg)/ورود پول ماه/
    # اختلاف سرانه/ارزش معاملات/بازده ماهانه/بازده ماهانه (قیمت-NAV)) عرض
    # بیشتری می‌گیرن تا هدرشون تک‌خط بمونه؛ نماد/NAV/درصدها کوتاه‌ترن.
    column_widths = [
        1.3, 1.3, 1.3, 1.0, 1.0,   # نماد | آخرین | NAV | % آخرین | % حباب
        1.3, 1.0, 1.2, 1.2,        # حباب ماه(Avg) | ورود پول | ورود پول ماه | سرانه خرید
        1.3, 1.2, 1.2, 1.4,        # اختلاف سرانه | ارزش معاملات | بازده ماهانه | بازده ماهانه (قیمت-NAV)
        1.0,                       # نماد (تکراری)
    ]
    ...
    fig.add_trace(
        go.Table(
            ...
            header=dict(
                ...
                font=dict(color="white", size=12, family=treemap_font_family), height=44,
            ),
            cells=dict(
